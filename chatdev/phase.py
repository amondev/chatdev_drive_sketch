...
class FindingDestination(Phase):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)
    ...
    def update_chat_env(self, chat_env) -> ChatEnv:
        if len(self.seminar_conclusion) > 0 and "<INFO>" in self.seminar_conclusion:
            chat_env.env_dict['destination'] = self.seminar_conclusion.split("<INFO>")[-1].lower().replace(".", "").strip()
        elif len(self.seminar_conclusion) > 0:
            chat_env.env_dict['destination'] = self.seminar_conclusion
        else:
            chat_env.env_dict['destination'] = "I have no idea"
        return chat_env
    

class PlanningTravel(Phase):
    def __init__(self, **kwargs):
        super().__init__(**kwargs)

    def update_phase_env(self, chat_env):
        self.phase_env.update({"task": chat_env.env_dict['task_prompt'], "destination": chat_env.env_dict['destination']})
    ...
    def update_chat_env(self, chat_env) -> ChatEnv:
        if len(self.seminar_conclusion) > 0 and "<INFO>" in self.seminar_conclusion:
            ...
            chat_env.env_dict['plan'] = "NO plan Sorry"
        return chat_env